...
-(void)setupCategoryButton{
    ...
    for (NSDictionary *dict in _items) {
        ...
        //创建button
        HWMenuCategoryButton *button = [HWMenuCategoryButton buttonWithType:UIButtonTypeCustom];
        button.frame = CGRectMake(buttonX, buttonY, buttonW, buttonH);
        button.name = dict[@"name"];
        [button setBackgroundImage:[UIImage imageNamed:[NSString stringWithFormat:@"%zd",i]] forState:UIControlStateNormal];
        int selCount = i + 11;
        [button setBackgroundImage:[UIImage imageNamed:[NSString stringWithFormat:@"%zd",selCount]] forState:UIControlStateHighlighted];
...
//点击分类按钮进入
-(void)clickCategoryBtn:(HWMenuCategoryButton *)btn{
//    NSLog(@"%@",btn.titleLabel.text);
    //按照具体点击的分类去获取数据
    //遍历数组 取出和按钮分类一致的选项
    for (NSDictionary *dict in _items) {
        if ([dict[@"name"] isEqualToString:btn.name]) {
            //能来到这里说明已经检索到正确的分类:
...
//    scrollView.showsVerticalScrollIndicator = NO;
    ...
    scrollView.contentSize = CGSizeMake(HWScreenW, 800);
